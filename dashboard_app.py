...
def calculate_session_stats(df):
    ...
    dist = df['distance_m'].max() if 'distance_m' in df else 0
    return max_s, avg_s, duration, dist


# --- פונקציית התצוגה (העיצוב המושקע) ---
def render_telemetry_view(race_data, is_live=False):
    """זו הפונקציה שמציירת את הגרפים היפים, נשתמש בה גם בלייב וגם בהיסטוריה"""
    if 'telemetry' in race_data:
        ...
        with tab1:
            max_s, avg_s, dur, dist = calculate_session_stats(df)

            # מטריקות מעוצבות
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Max Speed", f"{max_s:.1f} km/h")
            c2.metric("Avg Speed", f"{avg_s:.1f} km/h")
            c3.metric("Duration", f"{dur:.1f}s")
            c4.metric("Distance", f"{dist:.1f} m")

            # גרף מהירות (Area Chart)
            fig = px.area(df, x='time_str', y='speed_kph', title='Velocity Profile',
                          color_discrete_sequence=['#00CC96'])
            st.plotly_chart(fig, use_container_width=True)

            # גרף דוושות (שחסר בגרסה הקודמת והחזרתי אותו)
            fig2 = go.Figure()
        ...
        # בתוך render_telemetry_view, תחת tab2:
        ...
        with tab2:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"race_data_{race_data['id']}.csv",
                mime='text/csv',
            )
...
# --- התיקון לקפיצות (Fragment) ---
@st.fragment(run_every=1)
def live_monitor_widget(race_id):
    # שולף רק את המירוץ הספציפי הזה מהדאטהבייס
    ...
    st.title(f"🔴 LIVE: {race_data.get('type', 'Unknown')}")
    # קורא לפונקציית העיצוב היפה שלנו
    render_telemetry_view(race_data, is_live=True)


# --- לוגיקה ראשית (Sidebar וניווט) ---
...
if selection == "🔴 LIVE MONITOR" and live_race:
    # כאן אנחנו משתמשים בתיקון לקפיצות!
    live_monitor_widget(live_race['id'])
    ...
    if not category_races:
        ...
        if selected_id:
            # כאן מציגים את המירוץ שנבחר מההיסטוריה
            # משתמשים באותה פונקציית עיצוב יפה, אבל בלי הלייב
            race = next(r for r in category_races if r['id'] == selected_id)
            render_telemetry_view(race, is_live=False)